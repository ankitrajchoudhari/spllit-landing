          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 font-medium border-b-2 transition ${
              activeTab === tab.id
                ...
                : 'border-transparent text-gray-600 hover:text-accent-green'
            }`}
          >
            <tab.icon size={16} />
            {tab.label}
              ...
              <form onSubmit={handleSaveProvider} className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-accent-green"
                  >
                ...
                {/* Email */}
                <div>
                  <label className="block text-sm font-medium mb-2">Email Address (team@spllit)</label>
                  <input
                    type="email"
                ...
                {providerForm.name === 'gmail' ? (
                  <div>
                    <label className="block text-sm font-medium mb-2">Google Refresh Token</label>
                    <input
                      type="password"
                      value={providerForm.refreshToken}
                      onChange={(e) => setProviderForm({ ...providerForm, refreshToken: e.target.value })}
                      className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-accent-green text-xs"
            ...
            <div>
              ...
              <div className="grid gap-3">
                {providers.map((provider) => (
                  <div key={provider.id} className="bg-gray-50 p-4 rounded-lg flex justify-between items-center">
                    <div>
                      <p className="font-medium capitalize">{provider.name}</p>
                      <p className="text-sm text-gray-600">{provider.recipientEmail}</p>
                    </div>
                    <button
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            <div className="bg-gray-50 p-6 rounded-lg">
              ...
              <form onSubmit={handleSendBulkMail} className="space-y-4">
                ...
                {/* AI Prompt */}
                <div>
                  <label className="block text-sm font-medium mb-2">AI Prompt (Convert to Email)</label>
                  <textarea
                    value={mailForm.prompt}
                    onChange={(e) => setMailForm({ ...mailForm, prompt: e.target.value })}
                    className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:border-accent-green h-24 resize-none"
                ...
                {/* CSV Upload */}
                <div>
                  <label className="block text-sm font-medium mb-2">Upload CSV File (Email List)</label>
                  <input
                    type="file"
                    accept=".csv"
                  ...
                  <div key={campaign.id} className="bg-gray-50 p-4 rounded-lg">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{campaign.providerName}</p>
                      </div>
                      <span
                    ...
                    <p className="text-xs text-gray-500 mt-3">{new Date(campaign.createdAt).toLocaleString()}</p>
                  </div>
                ))
              )}
            </div>
